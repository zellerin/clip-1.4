...
and then run the experiments and report results to a text or Lisp file

** Known bugs
See [[file:doc/clip-1.4/release-notes.text][release notes]], plus:
- not Y2K compliant
- newer CL implementations (such as sbcl) need to be told how to advice functions if you want to use trigger based clips (partially fixed by using cl-advice)
